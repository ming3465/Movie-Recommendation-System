...
float user_pair_similarity(const RatingsTable& rt, std::int32_t i, std::int32_t j, Metric m) {
...
float item_pair_similarity(const RatingsTable& rt, std::int32_t i, std::int32_t j, Metric m) {
...
SimilarityMatrix build_similarity(std::int32_t n, ThreadPool& pool,
                                  float (*pair_fn)(const RatingsTable&, std::int32_t, std::int32_t,
                                                   Metric),
                                  const RatingsTable& rt, Metric m) {
    SimilarityMatrix out(n);
    if (n < 2)
        return out;
    pool.parallel_for(static_cast<std::size_t>(n), [&](std::size_t b, std::size_t e) {
        for (std::size_t i = b; i < e; ++i) {
            for (std::size_t j = i + 1; j < static_cast<std::size_t>(n); ++j) {
                const float s =
                    pair_fn(rt, static_cast<std::int32_t>(i), static_cast<std::int32_t>(j), m);
                out.set(static_cast<std::int32_t>(i), static_cast<std::int32_t>(j), s);
            }
        }
    });
    return out;
...
SimilarityMatrix::SimilarityMatrix(std::int32_t n) : n_(n) {
    if (n < 0)
        throw std::invalid_argument("SimilarityMatrix: negative size");
    const std::size_t slots =
        (n_ < 2) ? 0 : static_cast<std::size_t>(n_) * static_cast<std::size_t>(n_ - 1) / 2;
    upper_.assign(slots, 0.0f);
}

std::size_t SimilarityMatrix::flat_index(std::int32_t i, std::int32_t j, std::int32_t n) {
