...
TEST_CASE("build_user_similarity (Cosine): every entry matches direct cosine") {
    ...
    std::vector<Rating> raw = {
        {1, 100, 5.0f}, {1, 101, 4.0f}, {1, 102, 3.0f}, {2, 100, 4.0f},
        {2, 102, 2.0f}, {2, 103, 5.0f}, {3, 101, 3.0f}, {3, 103, 4.0f},
    };
    RatingsTable rt(raw);
    ThreadPool pool(2);
    ...
    for (std::int32_t i = 0; i < rt.num_users(); ++i) {
        for (std::int32_t j = i + 1; j < rt.num_users(); ++j) {
            ...
            const float direct =
                recsys::similarity::cosine(SparseVector{si.items, si.values, si.count},
...
TEST_CASE(
    "build_item_similarity (Pearson) and (AdjustedCosine) produce symmetric, in-range values") {
    std::vector<Rating> raw = {
        {1, 100, 5.0f}, {1, 101, 4.0f}, {1, 102, 3.0f}, {2, 100, 4.0f}, {2, 102, 2.0f},
        {2, 103, 5.0f}, {3, 101, 3.0f}, {3, 103, 4.0f}, {3, 100, 5.0f},
    };
    RatingsTable rt(raw);
    ThreadPool pool(2);
